...
public class BookingDAO {
    ...
    public boolean checkCourseCapacity(int courseId) {
        ...
        String sql = "SELECT " +
                "(SELECT COUNT(*) FROM booking WHERE course_id = ? AND booking_status = 'confirmed') AS booking_count, " +
    ...
    public int getAvailableSlots(int courseId) {
        String sql = "SELECT " +
                ...
                "(SELECT COUNT(*) FROM booking WHERE course_id = ? AND booking_status = 'confirmed') AS available";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, courseId);
            pstmt.setInt(2, courseId);
